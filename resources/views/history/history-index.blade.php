        ...
        @if ($videos->count() > 0)
            <div class="row justify-content-center">
                <form class="form-inline col-md-6 justify-content-center"
                        ...
                        method="post"
                        onsubmit="return confirm('Are you sure you want to delete the complete history ?')">
                    
                        @csrf
                        @method('DELETE')

                        <button type="submit" class="btn btn-secondary mb-2">Delete History</button>
                </form>
            </div>
            <hr>
        @endif
                                ...
                                <small class="text-muted">
                                    ...
                                    @auth
                                        @if ($video->user_id == auth()->user()->id || auth()->user()->administration_level > 0)
                                            <form action="{{route('history.destroy',$video->pivot->id)}}" method="post" onsubmit="return confirm('Are you sure you want to delete this video ?')">
                                                @csrf
                                                @method('DELETE')
