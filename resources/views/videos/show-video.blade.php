            ...
            <div class="mx-auto col-9">
                    ...
                    @foreach ($video->convertedVideos as $convertedVideo)
                        <video id="videoPlayer" controls style="{{ $video->Longitudinal == "0" ? "width:100%; height:90%;" : "width:900px; height:510px;"}}">
                            @if ($video->quality == 1080)
                                <source id="webm_source" src="{{ Storage::url($convertedVideo->webm_Format_1080) }}" type="video/webm">
                                <source id="mp4_source" src="{{ Storage::url($convertedVideo->mp4_Format_1080) }}" type="video/mp4">
                            @elseif($video->quality == 720)        
                                <source id="webm_source" src="{{ Storage::url($convertedVideo->webm_Format_720) }}" type="video/webm">
                                <source id="mp4_source" src="{{ Storage::url($convertedVideo->mp4_Format_720) }}" type="video/mp4">
                            @elseif($video->quality == 480)        
                                <source id="webm_source" src="{{ Storage::url($convertedVideo->webm_Format_480) }}" type="video/webm">
                                <source id="mp4_source" src="{{ Storage::url($convertedVideo->mp4_Format_480) }}" type="video/mp4">
                            @elseif($video->quality == 360)        
                                <source id="webm_source" src="{{ Storage::url($convertedVideo->webm_Format_360) }}" type="video/webm">
                                <source id="mp4_source" src="{{ Storage::url($convertedVideo->mp4_Format_360) }}" type="video/mp4">  
                            @else
                                <source id="webm_source" src="{{ Storage::url($convertedVideo->webm_Format_240) }}" type="video/webm">
                                <source id="mp4_source" src="{{ Storage::url($convertedVideo->mp4_Format_240) }}" type="video/mp4">  
                            @endif
                        </video>
                    @endforeach
                </div>
                ...
                <div class="interaction text-center mt-5">
                    ...
                    @foreach ($video->views as $view)
                        <span class="float-left">Number Of Views <span class="viewsNumber">{{$view->views_number}}</span></span>
                    @endforeach
                ...
                <div class="mt-4 px-2">
                    <div class="comments">
                        ...
                        <div>
                            <textarea name="comment" id="comment" rows="4" class="form-control" placeholder="add general comment"></textarea>
                            <button type="submit" class="btn btn-info mt-3 saveComment"> Comment </button>
                                                ...
                                                <div class="col-10">
                                                    @if (Auth::check())
                                                        @if ($comment->user_id == auth()->user()->id || auth()->user()->administration_level > 0)
                                                            @if(!auth()->user()->block)
                                                                <form action="{{ route('comment.destroy',$comment->id) }}" onsubmit="return confirm('Are you sure you want to delete this comment ?')" method="get">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                ...
                                                                <form action="{{ route('comment.edit',$comment->id) }}" method="get">
                                                                    @csrf
                                                                    @method('PATCH')
                                                                    <button type="submit" class="float-right"><i class="far fa-edit text-success fa-lg mr-3"></i></button>
                                                                </form>
                                                            @endif    
                                                        @endif
                                                    @endif
...
@section('script')
    <script>
        ...
        function changeQulity() {
            ...
            if (selected == '1080') {
                source = document.getElementById("webm_source").src = "{{ Storage::url($convertedVideo->webm_Format_1080) }}";
                source = document.getElementById("mp4_source").src = "{{ Storage::url($convertedVideo->mp4_Format_1080) }}";
            }
            
            else if (selected == '720') {
                source = document.getElementById("webm_source").src = "{{ Storage::url($convertedVideo->webm_Format_720) }}";
                source = document.getElementById("mp4_source").src = "{{ Storage::url($convertedVideo->mp4_Format_720) }}";
            }
                
            else if (selected == '480') {
                source = document.getElementById("webm_source").src = "{{ Storage::url($convertedVideo->webm_Format_480) }}";
                source = document.getElementById("mp4_source").src = "{{ Storage::url($convertedVideo->mp4_Format_480) }}";
            }
            else if (selected == '360') {
                source = document.getElementById("webm_source").src = "{{ Storage::url($convertedVideo->webm_Format_360) }}";
                source = document.getElementById("mp4_source").src = "{{ Storage::url($convertedVideo->mp4_Format_360) }}";
            }
            else {
                source = document.getElementById("webm_source").src = "{{ Storage::url($convertedVideo->webm_Format_240) }}";
                source = document.getElementById("mp4_source").src = "{{ Storage::url($convertedVideo->mp4_Format_240) }}";
            }
        ...
        $('.like').on('click', function(event) {
            ...
            if (AuthUser == "1") {
                ...
                var html='<div class="alert alert-danger">\
                            <ul>\
                                <li class="loginAlert">You have to login to like the video</li>\
                            </ul>\
                        </div>';
                ...
                var html='<div class="alert alert-danger">\
                            <ul>\
                                <li class="loginAlert">You are blocked from the like</li>\
                            </ul>\
                        </div>';
                $(".loginAlert").html(html);
    ...
    <script>
        $('.saveComment').on('click', function(event) {
            ...
            if (AuthUser == '1') {
                ...
                var html='<div class="alert alert-danger">\
                        <ul>\
                            <li>You have to login to like the video</li>\
                        </ul>\
                    </div>';
                ...
                var html='<div class="alert alert-danger">\
                        <ul>\
                            <li>Please write the comment</li>\
                        </ul>\
                    </div>';
                $(".commentAlert ").html(html);  
            }
